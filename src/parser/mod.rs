...
/// ```
///
pub struct Parser<E: PlyElement> {
...
//use std::marker::PhantomData;
//use std::io::{ Read, BufReader };
use crate::ply::{Ply, PlyElementSet};
use crate::ply::{ Header, Payload, Encoding };

impl<E: PlyElement+PlyElementSet> Parser<E> {
...
impl<E: PlyElement+PlyElementSet> Default for Parser<E> {
    fn default() -> Self {
        Self::new()
    }
}
...
use crate::ply::{ PlyElement, Version, ObjInfo, Comment, ElementDef, KeyMap, Addable };
/*
    ...
    fn __read_header<T: BufRead>(&self, reader: &mut T, location: &mut LocationTracker) -> Result<Header> {
        ...
        'readlines: loop {
            ...
            match line {
                ...
                Ok(Line::Property(p)) => if header_elements.is_empty() {
                        return parse_ascii_error(
                            location,
                            &line_str,
