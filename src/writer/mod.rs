...
/// ```
pub struct Writer<E: PlyElement> {
...
impl<E: PlyElementGet> Default for Writer<E> {
    fn default() -> Self {
        Self::new()
    }
}
...
impl<E: PlyElementGet> Writer<E> {
    ...
    fn write_binary_list<T: Write, D>(&self, list: &[D], out: &mut T, out_val: &dyn Fn(&mut T, &D) -> Result<usize>) -> Result<usize> {
        let mut written = 0;
        for v in list {
